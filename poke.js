...
var nudgeBox;
var queryServer = "http://inspirebeta.net/cgi-bin/";
...
function get_inspire_results() {
    $.getJSON(queryServer + 'queryinspire.py?query='+get_query()+'&callback=?',
              {},
              populate_results);
}
...
function create_feedback_functionality() {
    ...
    var expectedLabel = document.createTextNode('Yes!');
    ...
    var unexpectedLabel = document.createTextNode('No');
    ...
    var whyLabel = document.createTextNode('Why not?');
...
function notify() {
    ...
    search =  get_query();
    $.ajax(queryServer + 'feedback.py?yesNo='+yesNo+'&message='+why+'&search='+search);
